...
- **If using the `tala` layout engine and exporting SVG, remove the stray trailing `</mask>` and the following orphan `<text ...>...</text>` block if they appear at the end of the generated SVG, then ensure the file ends with a single valid `</svg>`**
