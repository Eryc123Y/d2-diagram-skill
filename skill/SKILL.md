...
- **If using the `tala` layout engine and exporting SVG, remove the stray trailing `</mask>` and the following orphan `<text ...>...</text>` block if they appear near the end of the generated SVG, while preserving the final `</svg></svg>` closing tags**
